...
def main():
    ...
    print("=" * 70)
    print("FSCV WINDOW GENERATOR (iPSC BINARY)")
    ...
    meta = meta[meta['label'].isin([0, 1])].copy()   # defensive filter — drops any stray non-binary label
    meta['label'] = meta['label'].astype(int)

    baseline_files  = set(meta[meta['label'] == 0]['plot_file'].unique())
    sero_files      = set(meta[meta['label'] == 1]['plot_file'].unique())
    available       = {f for f in os.listdir(PLOT_DIR) if f.lower().endswith(('.npy', '.txt', '.csv'))}

    print(f"\nFiles: {len(baseline_files)} baseline-only, {len(sero_files)} serotonin")
    ...
    # Process serotonin files (label=1)
    print("Processing serotonin files...")
    for fname in sorted(sero_files & available):
        try:
            arr = load_arr(os.path.join(PLOT_DIR, fname))
            arr = arr - arr[:, :bg_frames].mean(axis=1, keepdims=True)
            file_id  = fname[:-4]
    ...
    print(f"  Serotonin (1):  {(df['label']==1).sum()}")
