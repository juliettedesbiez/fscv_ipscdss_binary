"""
Test trained models on held-out test set (run train_models.py first).
Classes: 0=baseline, 1=serotonin

Usage: python test_models.py [all|rf|xgb|mlp] [--config fscv_config_ipsc.yaml]

Run once, at the end, when CV metrics look satisfactory — do not conflate
these held-out numbers with the CV metrics from train_models.py.
"""

import os, sys, json, pickle, argparse
import numpy as np, pandas as pd
import torch, torch.nn as nn
from utils_ipsc_binary import compute_metrics, print_metrics, select_models
from extract_features_ipsc_binary import extract, load_config
...
def main(selected=None):
    ...
    print("\nLoading TEST set...")
    test_meta = pd.read_csv(rf"{BASE}\windows_metadata_test_ipsc_binary.csv")
    y_test    = test_meta['label'].values

    print(f"Test set: {len(y_test)} samples")
    print(f"  Baseline (0):   {(y_test==0).sum()}")
    print(f"  Serotonin (1):  {(y_test==1).sum()}")

    # Extract features using same function as training (from extract_features.py)
