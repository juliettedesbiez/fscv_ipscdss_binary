...
import os, sys, pickle, argparse, numpy as np, pandas as pd
...
from utils_ipsc_binary import load_features, load_raw_for_features, compute_metrics, print_metrics, select_models, RANDOM_STATE
...
WINDOW_FRAMES = int(2.0 * _cfg['fscv_hz'])   # 20 frames, matches make_windows.py
N_VOLTAGE_PTS = 1100
...
def main(selected=None):
    ...
    if not os.path.exists(rf"{BASE}\features_ipsc_binary.csv"):
        print("features_ipsc_binary.csv not found — run extract_features.py first"); return
    ...
    class MLP(nn.Module):
        def __init__(self):
            ...
            self.net = nn.Sequential(
                ...
                nn.Linear(64, 2))          # 2 output classes (binary)
        def forward(self, x): return self.net(x)

    # Class weights: minority class (1 = serotonin/Event) boosted x3,
    # matching the winning v7 config (weighted CE + WeightedRandomSampler stacked)
    class_counts = np.bincount(y)
