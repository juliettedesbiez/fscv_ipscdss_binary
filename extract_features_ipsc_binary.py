...
def main():
    ...
    print(f"  Serotonin: {(meta['label']==1).sum()}")
    print(f"  Nothing: {(meta['label']==0).sum()}")

    # BALANCE: Keep all serotonin + balance_ratio× random nothing
    print(f"\nBalancing: all serotonin + {balance_ratio}× random nothing...")
    sero = meta[meta['label'] == 1]
    nothing = meta[meta['label'] == 0]

    np.random.seed(RANDOM_STATE)
    nothing = nothing.sample(min(len(nothing), balance_ratio * len(sero)), random_state=RANDOM_STATE)
    balanced = pd.concat([sero, nothing]).reset_index(drop=True)

    print(f"Balanced set: {len(balanced)} windows")
    print(f"  Serotonin: {(balanced['label']==1).sum()}")
    print(f"  Nothing: {(balanced['label']==0).sum()}")
    ...
    # train or test, never split — deliberate change from Phase 1's original
    # random split, which was kept only for Naweed comparability; this run
    # is testing pipeline generalisation, so group-aware is the correct
    # choice here, consistent with Phase 2 onward and the organoid pipeline)
    ...
    features.to_csv(rf"{BASE}\features_ipsc_binary.csv", index=False)

    # SAVE TEST METADATA for later
    test = balanced[balanced['split'] == 'test']
    test[['window_id', 'file_id', 'group_id', 'label']].to_csv(rf"{BASE}\windows_metadata_test_ipsc_binary.csv", index=False)

    print(f"\n✓ {BASE}\\features_ipsc_binary.csv ({len(features)} samples, {len(features.columns)-3} features)")
    print(f"✓ {BASE}\\windows_metadata_test_ipsc_binary.csv ({len(test)} samples)")
